...
// Fetches link previews through the Tauri HTTP plugin (server-side request, so
// not subject to CORS) and memoizes per URL. The in-flight promise is cached so
// concurrent widgets share one request; failures are evicted so a remount can retry.
