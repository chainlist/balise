<script lang="ts">
	...
	// Surface the note's date and reading time in the top navbar for as long as
	// this header is mounted; clear the slot on unmount (note closed, zen mode,
	// or the journal's headerless editors) so it never shows a stale note.
	$effect(() => {
		navbarState.center = meta;
		navbarState.grabbable = true;
		return () => {
			navbarState.center = null;
			navbarState.grabbable = false;
		};
	});
...
{#snippet meta()}
	<span class="flex items-center gap-2 font-mono text-sm text-muted-foreground">
...
{#if tags.length}
	<div class="mx-auto flex w-full max-w-175 flex-wrap gap-1.5 px-10 pt-4 pb-2 select-none">
